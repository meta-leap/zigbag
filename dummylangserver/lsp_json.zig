...
pub fn load(comptime T: type, mem: *std.heap.ArenaAllocator, from: *const std.json.Value) std.mem.Allocator.Error!?T {
        ...
        return switch (from.*) {
            ...
            .Array => |jlist| load_all: {
                ...
                for (jlist.items[0..jlist.len]) |*item, i|
                    arr[i] = (try load(types.JsonAny, mem, item)) orelse return null;
            ...
            .Object => |jmap| load_all: {
                ...
                while (iter.next()) |pair|
                    _ = try obj.put(pair.key, (try load(types.JsonAny, mem, &pair.value)) orelse return null);
                break :load_all .{ .array = &[_]types.JsonAny{} };
            },
        };
    return null;
