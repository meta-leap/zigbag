    ...
    if (T == types.IntOrString)
        ...
        return switch (from.*) {
            ...
            .Array => |jlist| load_all: {
                var arr = mem.allocator.alloc(types.JsonAny, jlist.len) catch unreachable;
                for (jlist.items[0..jlist.len]) |*item, i|
                    arr[i] = unmarshal(types.JsonAny, mem, item) orelse return null;
            ...
            .Object => |jmap| load_all: {
                ...
                while (iter.next()) |item|
                    _ = obj.put(item.key, unmarshal(types.JsonAny, mem, &item.value) orelse return null) catch unreachable;
                break :load_all .{ .array = &[_]types.JsonAny{} };
            },
        }
