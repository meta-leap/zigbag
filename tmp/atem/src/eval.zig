    ...
    restep: while (true) {
        ...
        switch (cur.stash.items[@intCast(usize, cur.pos)]) {
            ...
            .ArgRef => |it| {
                ...
                cur.stash.items[@intCast(usize, cur.pos)] = stash_lookup.items[@intCast(usize, @intCast(isize, stash_lookup.len) + it)]; // TODO: can turn 2 intCasts into 1, via -it
                if (cur.pos == idx_callee)
                    continue :restep
                else
                    cur.pos -= 1;
            },
            ...
            .Call => |it| if (it.IsClosure == 0) {
                ...
                try callargs.appendSlice(it.Args);
                ...
                if (cur.num_args == 0) {
                    ...
                    if (maybefuncdef) |f| {
                        ...
                        if (cur.num_args == 0) {
                            ...
                            num_args_done = if (0 == call.IsClosure) 0 else (num_args_done + call.Args.len);
                            continue :restep;
                        } else if (!f.allArgsUsed) {
                            const until = if (cur.num_args < idx_callee) cur.num_args else idx_callee;
                            var i = num_args_done;
                            while (i < until) : (i += 1) if (0 == f.Args[i]) {
                    ...
                    var result: Expr = undefined; // TODO expressionify
            ...
            if (cur.done_args) {
                {
                    ...
                    if (diff < 1) {
                        cur.stash.len -= cur.num_args;
                        cur.stash.items[cur.stash.len - 1] = result;
                    } else {
                        ...
                        //     continue :restep;
                        // } else {
                        const args = try std.mem.dupe(mem, Expr, cur.stash.items[0..idx_callee]); // could skip dupe if it weren't for the 0 position =/
                        cur.stash.len = 1;
                ...
                cur.pos = @intCast(isize, cur.stash.len) - 1;
            } else if (cur.pos < 0 or cur.pos < (@intCast(isize, idx_callee) - @intCast(isize, cur.num_args))) {
